...
def _manifest_summary(path):
    ...
    section_summary = final_outputs.get("section_summary", {})
    return {
        "manifest_path": str(path),
        "exists": True,
        ...
        "section_backend_name": section_summary.get("backend_name"),
        "section_apply_count": section_summary.get("apply_count"),
        "section_cache_hit_count": section_summary.get("cache_hit_count"),
        "section_cache_hit_rate": section_summary.get("cache_hit_rate"),
        "section_cache_size": section_summary.get("cache_size"),
        "section_fallback_count": section_summary.get("fallback_count"),
        "section_decoder_failure_count": section_summary.get(
            "decoder_failure_count"
        ),
        "section_ldpc_import_error": section_summary.get(
            "ldpc_import_error"
        ),
...
def _pooled_diagnostic_row(path, lattice_size, q_value):
    ...
    row = {
        ...
        "section_backend_name": str(
            _as_python_scalar(result.get("section_backend_name", "unknown"))
        ),
        "section_apply_count": int(
            _as_python_scalar(result.get("section_apply_count", 0))
        ),
        "section_cache_hit_count": int(
            _as_python_scalar(result.get("section_cache_hit_count", 0))
        ),
        "section_cache_hit_rate": float(
            _as_python_scalar(result.get("section_cache_hit_rate", 0.0))
        ),
        "section_cache_size": int(
            _as_python_scalar(result.get("section_cache_size", 0))
        ),
        "section_fallback_count": int(
            _as_python_scalar(result.get("section_fallback_count", 0))
        ),
        "section_decoder_failure_count": int(
            _as_python_scalar(
                result.get("section_decoder_failure_count", 0)
            )
        ),
    }
...
def _summarize_diagnostics(rows):
    ...
    summary = {
        ...
        "section": {
            "backend_names": sorted(
                {
                    row.get("section_backend_name")
                    for row in rows
                    if row.get("section_backend_name")
                }
            ),
            "total_apply_count": int(
                sum(row.get("section_apply_count", 0) for row in rows)
            ),
            "total_cache_hit_count": int(
                sum(row.get("section_cache_hit_count", 0) for row in rows)
            ),
            "total_cache_size": int(
                sum(row.get("section_cache_size", 0) for row in rows)
            ),
            "total_fallback_count": int(
                sum(row.get("section_fallback_count", 0) for row in rows)
            ),
            "total_decoder_failure_count": int(
                sum(
                    row.get("section_decoder_failure_count", 0)
                    for row in rows
                )
            ),
        },
    ...
    if summary["section"]["total_apply_count"] > 0:
        summary["section"]["overall_cache_hit_rate"] = (
            summary["section"]["total_cache_hit_count"]
            / summary["section"]["total_apply_count"]
        )
    else:
        summary["section"]["overall_cache_hit_rate"] = None
    return summary

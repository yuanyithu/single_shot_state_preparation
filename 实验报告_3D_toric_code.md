...
## 2026-04-26 3D `q=0.0100/0.0500/0.1000` 高采样窄窗

摘要：
- 做什么：三台节点并行跑 `q=0.0100,0.0500,0.1000`，统一窗口 `p=0.180,0.185,...,0.220`，`L=3,4,5`，每个节点每个 q `384` disorder；池化后每个 q 为 `1152` disorder。
- 产物：`exp24a/b/c` 为三份独立远端 run，`exp25_q001_q050_q100_p018_022_dense_combined_summary` 为池化分析目录。
- 结论：按正确方向判读，`q=0.0500` 在本窗口内给出最清楚的三尺寸 crossing 信号，共同 crossing window `p≈0.2032~0.2121`；`q=0.0100` 到 `p=0.220` 仍主要表现为大码更好，threshold 应在更右侧；`q=0.1000` 从 `p=0.180` 起就表现为大码更差，threshold 应在更左侧。
- 重要限制：虽然 disorder error bar 明显变小，但 `q=0.0100/0.0500` 的 L=4/5 MCMC mixing 诊断仍差，尤其 ESS 和 multi-start spread；因此本轮给出强窗口证据，而不是最终高精度 threshold 宣称。
...
```text
data/3d_toric_code/with_measurement_noise/exp24a_q001_q050_q100_p018_022_dense_20260426_nd1
data/3d_toric_code/with_measurement_noise/exp24b_q001_q050_q100_p018_022_dense_20260426_nd2
data/3d_toric_code/with_measurement_noise/exp24c_q001_q050_q100_p018_022_dense_20260426_nd3
data/3d_toric_code/with_measurement_noise/exp25_q001_q050_q100_p018_022_dense_combined_summary
```

完成状态：

```text
exp24a nd-1: q=0.0100/0.0500/0.1000 均 2592/2592 chunks, failed=0
exp24b nd-2: q=0.0100/0.0500/0.1000 均 2592/2592 chunks, failed=0
exp24c nd-3: q=0.0100/0.0500/0.1000 均 2592/2592 chunks, failed=0
```

主看图：

- [exp25 pooled q_top overview](data/3d_toric_code/with_measurement_noise/exp25_q001_q050_q100_p018_022_dense_combined_summary/exp25_pooled_sem95_overview.png)
- [exp25 pooled gap overview](data/3d_toric_code/with_measurement_noise/exp25_q001_q050_q100_p018_022_dense_combined_summary/exp25_pooled_gap_overview.png)
- [q=0.0500 exp21+exp24 shared-p overlay](data/3d_toric_code/with_measurement_noise/exp25_q001_q050_q100_p018_022_dense_combined_summary/q0p0500_exp21_exp24_shared_p_overlay.png)
- [machine-readable summary](data/3d_toric_code/with_measurement_noise/exp25_q001_q050_q100_p018_022_dense_combined_summary/exp25_pooled_summary.json)

### 数值摘要

`q=0.0100`，`1152` disorder：

```text
p      L3              L4              L5              L3-L4    L4-L5
0.180  0.5089±0.0169  0.6472±0.0172  0.7570±0.0160  -0.1383  -0.1097
0.185  0.4702±0.0166  0.5928±0.0176  0.7007±0.0171  -0.1226  -0.1079
0.190  0.4391±0.0163  0.5457±0.0177  0.6352±0.0176  -0.1066  -0.0895
0.195  0.3985±0.0158  0.4905±0.0177  0.5774±0.0177  -0.0919  -0.0869
0.200  0.3685±0.0152  0.4363±0.0173  0.5083±0.0176  -0.0678  -0.0719
0.205  0.3329±0.0146  0.3812±0.0165  0.4417±0.0172  -0.0482  -0.0605
0.210  0.2985±0.0136  0.3320±0.0155  0.3784±0.0164  -0.0335  -0.0464
0.215  0.2676±0.0127  0.2855±0.0146  0.3200±0.0154  -0.0179  -0.0345
0.220  0.2411±0.0120  0.2436±0.0132  0.2672±0.0140  -0.0025  -0.0236
```

`q=0.0500`，`1152` disorder：

```text
p      L3              L4              L5              L3-L4    L4-L5
0.180  0.4394±0.0164  0.5040±0.0181  0.5398±0.0177  -0.0646  -0.0358
0.185  0.4002±0.0159  0.4552±0.0178  0.4884±0.0179  -0.0550  -0.0332
0.190  0.3661±0.0155  0.4078±0.0172  0.4395±0.0178  -0.0417  -0.0317
0.195  0.3316±0.0147  0.3659±0.0167  0.3851±0.0172  -0.0343  -0.0191
0.200  0.3041±0.0141  0.3289±0.0159  0.3361±0.0164  -0.0248  -0.0073
0.205  0.2780±0.0134  0.2914±0.0151  0.2873±0.0152  -0.0134  +0.0041
0.210  0.2517±0.0127  0.2560±0.0143  0.2407±0.0140  -0.0042  +0.0153
0.215  0.2243±0.0116  0.2185±0.0132  0.1994±0.0126  +0.0058  +0.0191
0.220  0.1974±0.0109  0.1870±0.0120  0.1659±0.0114  +0.0104  +0.0211
```

`q=0.1000`，`1152` disorder：

```text
p      L3              L4              L5              L3-L4    L4-L5
0.180  0.2366±0.0130  0.1567±0.0098  0.0922±0.0065  +0.0799  +0.0645
0.185  0.2164±0.0123  0.1349±0.0090  0.0775±0.0058  +0.0815  +0.0574
0.190  0.1958±0.0115  0.1164±0.0083  0.0657±0.0052  +0.0793  +0.0507
0.195  0.1747±0.0107  0.1021±0.0074  0.0544±0.0047  +0.0726  +0.0476
0.200  0.1546±0.0098  0.0869±0.0067  0.0453±0.0040  +0.0676  +0.0416
0.205  0.1379±0.0092  0.0725±0.0058  0.0380±0.0036  +0.0654  +0.0345
0.210  0.1233±0.0084  0.0612±0.0049  0.0298±0.0028  +0.0621  +0.0313
0.215  0.1109±0.0077  0.0512±0.0042  0.0240±0.0022  +0.0597  +0.0273
0.220  0.1005±0.0072  0.0431±0.0037  0.0189±0.0018  +0.0574  +0.0241
```

### Crossing 判读

- `q=0.0100`：`L3-L4` 到 `p=0.220` 只接近 0，`L4-L5` 仍为负；按方向规则，这说明 `0.180~0.220` 主要仍是 threshold 左侧，下一轮若继续 q=0.01，应向 `0.220~0.240` 集中补点，而不是回到更左侧。
- `q=0.0500`：`L4-L5` 在 `p≈0.203` 附近翻号，`L3-L4` 在 `p≈0.212` 附近翻号，自动共同 crossing window 为 `p≈0.2032~0.2121`，代表点 `p≈0.2076`。把旧 `exp21` 的共享点并入后，`1440` disorder 的共享点 gap 也给出 `L4-L5` 在 `0.20~0.21`、`L3-L4` 在 `0.21` 附近翻号。
- `q=0.1000`：两条 gap 在全窗口均为正，即大码 `q_top` 更低；因此 `p=0.180` 已经在 high-p side。若要找 q=0.10 threshold，应向左扫，例如 `p=0.120~0.180`，不要继续在 `0.18~0.22` 加样本。

### Convergence 诊断

```text
q=0.0100: 0/27 points passed gate
  L3: spread_max≈0.0289, rhat_max≈1.249, ESS_min≈6.82, swap_min≈0.443
  L4: spread_max≈0.0404, rhat_max≈1.203, ESS_min≈5.60, swap_min≈0.236
  L5: spread_max≈0.0752, rhat_max≈2.027, ESS_min≈3.99, swap_min≈0.093

q=0.0500: 0/27 points passed gate
  L3: spread_max≈0.0303, rhat_max≈1.024, ESS_min≈24.65, swap_min≈0.450
  L4: spread_max≈0.0395, rhat_max≈1.196, ESS_min≈6.23, swap_min≈0.243
  L5: spread_max≈0.0682, rhat_max≈1.168, ESS_min≈5.75, swap_min≈0.102

q=0.1000: 13/27 points passed gate
  L3: spread_max≈0.0243, rhat_max≈1.003, ESS_min≈165.70, swap_min≈0.467
  L4: spread_max≈0.0263, rhat_max≈1.005, ESS_min≈143.57, swap_min≈0.269
  L5: spread_max≈0.0283, rhat_max≈1.020, ESS_min≈24.32, swap_min≈0.127
```

这里的 gate 对 `min ESS` 很严，会被少量难混合 disorder 拉低；但 `q=0.0100/0.0500` 的 L=4/5 spread 与 R-hat 也确实偏差，所以不能只看 disorder error bar。后续提高精度不应继续盲目加 disorder，而应优先改善 mixing：增加 `num_replicas_per_start`、提高 `PT_NUM_TEMPERATURES`、提高 measurement/burn-in 或专门针对 L5 的参数。

### 下一步建议

```text
q = 0.0100:
  重点窗口 p = 0.220,0.225,0.230,0.235,0.240
  目标是确认 L4-L5 crossing 是否在 0.22 右侧出现。

q = 0.0500:
  重点窗口 p = 0.200,0.205,0.210,0.215
  若要做最终定值，先改善 L5 mixing，再补 disorder；当前最佳候选 p_threshold ≈ 0.208。

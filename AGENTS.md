...
- exp103--exp106 全部完成并冻结，均为可比证据与 equality gate 的只读对照，不再开新计算。接手先读 `data/expander_code/exp106/status.md`，再读 `data/expander_code/exp106/validation/INDEX.md`。当前无进行中的 expander 主线；新开实验须另立编号目录与契约。
...
- 读出阈值现状：exp104 在 `q=0` 认证 crossing（`p_cross=0.05512`），exp105 在 `q=0.05`、exp106 在 `q=0.01` 都认证全程无 crossing，故 `q_c` 严格落在 `(0, 0.01)`。再收窄需另立契约；不得把带宽不足写成物理结论，也不得把三个点加一个区间说成阈值曲线。

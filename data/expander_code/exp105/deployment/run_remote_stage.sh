...
# exp104 runs as a single stage. Each step is skipped if its immutable artifact
...
run_root=$3
config=data/expander_code/exp104/config/ensemble_mc.remote.v1.json
module=data.expander_code.exp104.exp104_pipeline.remote_cli
common=(
  --config "$config"
  --run-root "$run_root"
...
echo "exp104 remote stage complete"

...
REMOTE_DEPLOYMENT_SCHEMA = "exp105.remote_deployment.v1"
...
def runtime_identity(config, verify_source=False, repo_root=None):
    ...
    if verify_source:
        ...
        if diff.returncode != 0 or dirty:
            raise ValueError("exp105 source differs from the frozen source commit")
    return actual


def require_canonical_python():
    if sys.version_info[:3] != (3, 12, 12):
        raise ValueError("exp105 formal statistics require Python 3.12.12")
...
def _require_file_matches_commit(root, path, commit):
    ...
    if path.read_bytes() != committed:
        raise ValueError(f"formal exp105 artifact differs from source commit: {relative}")
...
def verify_frozen_repository(config_path, repo_root=None):
    ...
    if dirty:
        raise ValueError("formal exp105 contract/config/source has uncommitted changes")
    subprocess.run(
    ...
    if pushed.returncode != 0:
        raise ValueError("formal exp105 HEAD is not present in the configured upstream")
...
def verify_remote_deployment(
    ...
    package_dir = source_root / "data" / "expander_code" / "exp105" / "exp105_pipeline"
    ...
    config_path = source_root / "data" / "expander_code" / "exp105" / "config" / "ensemble_mc.remote.v1.json"

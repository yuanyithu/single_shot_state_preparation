"""Independent re-verification of a published exp105 aggregate.
...
def _load_npz(path):
    with np.load(path, allow_pickle=False) as data:
        ...
        if missing or extra:
            raise ValueError("aggregate NPZ fields do not match exp105.aggregate.v1")
        payload = {}
...
def load_exp105_crossing(path_or_payload, config, require_complete=True):

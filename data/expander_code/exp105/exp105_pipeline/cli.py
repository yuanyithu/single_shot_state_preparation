"""Local command line for exp105: config authoring, tasks, replay, aggregation."""
...
def command_write_config(args):
    ...
    if args.remote:
        if args.phase == "pilot":
            raise ValueError("the locating pilot is a local phase and has no remote config")
        remote = _remote_config(base)
        atomic_json(config_dir / "noisy_mc.remote.v1.json", remote)
...
def build_parser():
    parser = argparse.ArgumentParser(description="exp105 local pipeline")

...
from data.expander_code.exp105.exp105_pipeline.raw import task_counts
from data.expander_code.exp105.exp105_pipeline.preflight import _synthetic_raw
from data.expander_code.exp105.exp105_pipeline.raw import raw_filename, save_raw
...
def test_a_schema_mismatch_is_recorded_as_an_unexpected_error(tmp_path, frozen_config):
    _write_task(tmp_path, frozen_config, 8, 0, schema_version="exp105.raw.v0")
    aggregate = aggregate_scan(tmp_path, frozen_config)
...
def test_a_q_mismatch_is_recorded_as_an_unexpected_error(tmp_path, frozen_config):
    """exp105 is a fixed-q experiment; raw from another q must never merge."""
    _write_task(tmp_path, frozen_config, 8, 0, q_token="0.02")
    aggregate = aggregate_scan(tmp_path, frozen_config)

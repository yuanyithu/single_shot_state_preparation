...
from data.expander_code.exp105.exp105_pipeline import remote_cli
...
TASKS_PER_M = tasks_per_m(CODES_PER_M, CODES_PER_TASK)
TOTAL_CODES = sum(CODES_PER_M.values())
from data.expander_code.exp105.exp105_pipeline.preflight import estimate_resources
from data.expander_code.exp105.exp105_pipeline.raw import raw_filename
...
def test_replay_report_validation_requires_full_coverage(remote_config):
    ...
    from data.expander_code.exp105.exp105_pipeline.config import block_code_indices
...
def test_run_root_must_sit_under_the_frozen_run_root(remote_config, tmp_path):
    resolved = remote_cli.resolve_remote_run_root(
        "~/.single_shot/runs/exp105_noisy_v1_001", remote_config,
    )
    assert resolved.name == "exp105_noisy_v1_001"
    with pytest.raises(ValueError):
...
def test_remote_commands_refuse_the_local_config(production_config, pilot_config):
    assert production_config["schema_version"] == "exp105.config.v1"
    assert pilot_config["schema_version"] == "exp105.config.pilot.v1"

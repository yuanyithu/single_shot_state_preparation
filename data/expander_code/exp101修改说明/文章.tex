...
\begin{equation}
\begin{aligned}
    m_u(s,\eta) &= \frac{1}{\mathcal Z(s,\sigma_\eta)}\frac{1}{2^{r_z'}}\sum_{\sigma\in \im H_Z, b\in B^1, [l] \in H^1} O_u(c(\sigma)+c(\sigma_\eta)+b+l) q(\sigma+s)p(c(\sigma)+c(\sigma_\eta)+b+l)\\
    &=\frac{1}{\Pr(s,\sigma_\eta)}\sum_{[l] \in H^1}O_u([r(\sigma+\sigma_\eta)]+[l])\Pr(s,\sigma_\eta,[l])\\
    &=\frac{1}{\Pr(s,\sigma_\eta)}\sum_{[l] \in H^1}-1^{u\cdot ([r(\sigma+\sigma_\eta)+c(\sigma+\sigma_\eta)]+[l])}\Pr(s,\sigma_\eta,[l]).
\end{aligned}
\end{equation}
Since the choice of $c(\cdot)$ could be arbitrary, we let $c(\cdot)=r(\cdot)$ and have
...
We now fix a linear section
\[
    r:\operatorname{im}H_Z\to C^1,
    \qquad
    H_Zr(\sigma)=\sigma .
\]
This choice is allowed for the preparation representative, because the
preparation starts from the logical \(+\) state, so changing the representative
of a physical syndrome by an element of \(Z^1=B^1\oplus H^1\) does not change
the prepared state. In the following we choose the preparation representative
and the final syndrome representative consistently as
\[
    c_{\rm prep}(\sigma)=r(\sigma),
    \qquad
    c_{\rm fin}(\sigma_\eta)=r(\sigma_\eta),
    \qquad
    \sigma_\eta:=H_Z\eta .
\]
Then
\[
    \mathcal Z_{[l]}(s,\sigma_\eta)
    =
    \sum_{\sigma\in\operatorname{im}H_Z}
    \sum_{b\in B^1}
    q(\sigma+s)\,
...
and
\[
    \mathcal Z(s,\sigma_\eta)
    =
    \sum_{[l]\in H^1}\mathcal Z_{[l]}(s,\sigma_\eta).
\]
Let
\[
    e=r(\sigma)+r(\sigma_\eta)+b+l .
\]
Then
\[
    H_Ze=\sigma+\sigma_\eta .
\]
Define
\[
    \ell_r(e):=e+r(H_Ze).
...
\[
    [\ell_r(e)]=[l].
\]
For a logical character \(u\in H_1\), the observable
\[
    O_u(e):=(-1)^{u\cdot[\ell_r(e)]}
\]
therefore satisfies
\[
    O_u(e)=(-1)^{u\cdot[l]}.
...
\begin{aligned}
    ...
    \sum_{b\in B^1}
    \sum_{[l]\in H^1}
    O_u\!\left(r(\sigma)+r(\sigma_\eta)+b+l\right)
    q(\sigma+s)
    p\!\left(r(\sigma)+r(\sigma_\eta)+b+l\right)
    \\
    &=
...
\end{aligned}
\]
Thus \(m_u(s,\sigma_\eta)\) is exactly the Fourier transform of the posterior
distribution on logical sectors.

The factorized expression
\[
    O_u(c)=\prod_{i=1}^n(-1)^{\Lambda_{u,i}c_i}
\]
uses the linearity of the section \(r\). If \(r\) is not assumed to be linear,
then this site-factorized form is no longer valid. However, the disorder
dependent Mattis factor can still be separated. We should replace the
factorized observable by
\begin{equation}\label{eq:correct-Ou-nonlinear}
    O_u(c):=
    (-1)^{\langle \bar z_u,\ c+r(H_Zc)\rangle}.
\end{equation}
For a fixed disorder sample \((s,\eta)\), with
\[
    \sigma_\eta:=H_Z\eta,
\]
define
\begin{equation}\label{eq:xi-nonlinear}
    \xi_u(\eta):=
    (-1)^{\langle \bar z_u,\ \eta+r(\sigma_\eta)\rangle}.
\end{equation}
Then the full logical-sector observable is
\begin{equation}\label{eq:Ou-full-nonlinear}
    O_u(c;\eta)
    :=
    \xi_u(\eta) O_u(c)
    =
    ...
    \langle
    \bar z_u,\,
    c+\eta+r(H_Zc)+r(\sigma_\eta)
    \rangle
    }.
\end{equation}
This definition uses the same section \(r:\im H_Z\to C^1\) as in the
logical-sector decomposition, but it does not require \(r\) to be linear.
Indeed, decompose
\begin{equation}\label{eq:sector-decomp-r}
    c+\eta=r(\sigma)+r(\sigma_\eta)+b+l,
    \qquad
    \sigma=H_Zc,
    \qquad
    b\in B^1,\quad [l]\in H^1 .
\end{equation}
Then
\[
\begin{aligned}
    c+\eta+r(H_Zc)+r(\sigma_\eta)
    &=
    r(\sigma)+r(\sigma_\eta)+b+l+r(\sigma)+r(\sigma_\eta)\\
    &=
    b+l .
\end{aligned}
\]
Therefore
\begin{equation}\label{eq:Ou-reads-logical-sector}
    O_u(c;\eta)
    =
    (-1)^{\langle \bar z_u,b+l\rangle}
    =
    (-1)^{u\cdot[l]}.
\end{equation}
Consequently, the thermal average gives exactly the Fourier transform of the
posterior logical-sector distribution:
\begin{equation}\label{eq:m-u-fourier-corrected}
\begin{aligned}
    m_u(s,\eta)
    &:=
    \frac{1}{\mathcal Z(s,\sigma_\eta)}
...
\end{aligned}
\end{equation}
Equivalently, if one uses the disorder-independent observable \(O_u(c)\) in
\eqref{eq:correct-Ou-nonlinear}, its thermal average differs from \(m_u\) only
by the sign \(\xi_u(\eta)\):
\begin{equation}
    \frac{1}{\mathcal Z(s,\sigma_\eta)}
    \sum_{c\in C^1}
    O_u(c)\,
    ...
    m_u(s,\eta).
\end{equation}
Hence the sign \(\xi_u(\eta)\) cancels in \(m_u^2\), and therefore it does not
affect \(q_{\mathrm{top}}\).

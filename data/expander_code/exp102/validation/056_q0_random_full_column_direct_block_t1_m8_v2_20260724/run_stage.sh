...
case "$stage" in
  ...
  *) echo "invalid direct-block T1 v2 stage: $stage" >&2; exit 64 ;;
esac
[[ ${EXP102_SOURCE_COMMIT:-} =~ ^[0-9a-f]{40}$ ]] || {
  echo "direct-block T1 v2 stage must run inside run_verified_source.sh" >&2
  exit 66
}
[[ $# -ge 1 ]] || exit 64
case "$stage:$1" in
  ...
  *) echo "workflow action does not match direct-block T1 v2 stage" >&2; exit 68 ;;
esac

mkdir -p "$stage_dir" "$(dirname "$log_file")"
for marker in RUNNING SUCCESS FAILED; do
  [[ ! -e "$stage_dir/$marker" ]] || {
    echo "immutable direct-block T1 v2 marker exists: $stage_dir/$marker" >&2
    exit 69
  }
done
[[ ! -e "$log_file" ]] || {
  echo "immutable direct-block T1 v2 log exists: $log_file" >&2
  exit 69
}
...
python -m data.expander_code.exp102.validation.056_q0_random_full_column_direct_block_t1_m8_v2_20260724.workflow \
  "$@" >"$log_file" 2>&1

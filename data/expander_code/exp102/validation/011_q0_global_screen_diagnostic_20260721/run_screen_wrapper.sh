...
if [[ $# -lt 4 ]]; then
  echo "usage: run_screen_wrapper.sh STAGE_DIR LOG_FILE STAGE_FINGERPRINT COMMAND [ARGS...]" >&2
  exit 64
fi
...
for marker in RUNNING SUCCESS FAILED; do
  if [[ -e "$stage_dir/$marker" ]]; then
    ...
    if [[ "$recorded" != "$stage_fingerprint" ]]; then
      echo "diagnostic marker identity conflict: $stage_dir/$marker" >&2
      exit 74
    fi
  fi
done
...
if [[ -e "$stage_dir/RUNNING" ]]; then
  echo "stale diagnostic RUNNING marker requires a fresh deployment" >&2
  exit 75
fi
if [[ -e "$stage_dir/FAILED" ]]; then
  echo "diagnostic FAILED marker requires a fresh deployment" >&2
  exit 76
fi

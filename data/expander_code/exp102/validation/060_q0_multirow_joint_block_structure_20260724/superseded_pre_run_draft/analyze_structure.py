...
def summarize_plan(candidate_id, variable_count, scopes, gates, parameter):
...
def load_inputs():
    ...
    predecessor = json.loads((ROOT.parent /
        "059_q0_hybrid_row_column_local_pilot_20260724/local_run_v0/pilot_report.json"
    ).read_text(encoding="ascii"))
    verify_self_hash(predecessor, "report_sha256")
    if predecessor["report_sha256"] != config["predecessor_report_sha256"]:
...
def main():
    ...
    config, context, H_GLOBAL = load_inputs()
    gates = config["gates"]
    results = []
    for candidate in config["candidates"]:
        ...
        if candidate["kind"] == "multirow":
            ...
            scopes = factor_scopes_multirow(H_GLOBAL, row_count)
            block_count = math.comb(H_GLOBAL.shape[0], row_count)
            plans = [summarize_plan(
                candidate_id, H_GLOBAL.shape[0] * row_count, scopes, gates,
                row_count,
            )]
        elif candidate["kind"] == "row_column_cross":
            block_count = H_GLOBAL.shape[0] * H_GLOBAL.shape[0]
            plans = [
                summarize_plan(
                    candidate_id, 2 * H_GLOBAL.shape[0] - 1,
                    factor_scopes_row_column(H_GLOBAL, selected_column), gates,
                    selected_column,
                )
                for selected_column in range(H_GLOBAL.shape[0])
            ]
        else:
    ...
    core = {
        ...
        "h_sha256": matrix_sha256(H_GLOBAL),
        "results": results,
    ...
    OUTPUT.write_text(canonical(core) + "\n", encoding="ascii")

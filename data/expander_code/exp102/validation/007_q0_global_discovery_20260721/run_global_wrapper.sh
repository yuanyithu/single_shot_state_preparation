...
if [[ $# -lt 4 ]]; then
  echo "usage: run_global_wrapper.sh STAGE_DIR LOG_FILE STAGE_FINGERPRINT COMMAND [ARGS...]" >&2
  exit 64
fi
...
for marker in RUNNING SUCCESS FAILED; do
  if [[ -e "$stage_dir/$marker" ]]; then
    ...
    if [[ "$recorded" != "$stage_fingerprint" ]]; then
      echo "global stage marker identity conflict: $stage_dir/$marker" >&2
      exit 74
    fi
  fi
done
...
if [[ -e "$stage_dir/RUNNING" ]]; then
  echo "stale global RUNNING marker requires a fresh deployment" >&2
  exit 75
fi

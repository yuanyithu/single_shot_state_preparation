...
from data.expander_code.exp106.exp106_pipeline.aggregate import ARRAY_FIELDS, SCALAR_FIELDS
from data.expander_code.exp106.exp106_pipeline.crossing import CERTIFIED
from data.expander_code.exp106.exp106_pipeline.loader import load_exp106_crossing


def test_loader_accepts_the_complete_aggregate(complete_aggregate_factory, pilot_config):
    payload = complete_aggregate_factory()
    accepted = load_exp106_crossing(payload, pilot_config)
...
def test_loader_round_trips_through_an_npz_file(
    tmp_path, complete_aggregate_factory, pilot_config,
):
    payload = complete_aggregate_factory()
    ...
    accepted = load_exp106_crossing(path, pilot_config)
...
def test_loader_rejects_a_relabelled_terminal_status(
    ...
    payload = complete_aggregate_factory()
    payload["terminal_status"] = "EXP106_NO_CERTIFIED_CROSSING"
    rehash_aggregate(payload)
...
def test_loader_rejects_a_foreign_config(
    complete_aggregate_factory, pilot_config, foreign_config,
):
    assert foreign_config["config_sha256"] != pilot_config["config_sha256"]
    payload = complete_aggregate_factory()
    with pytest.raises(ValueError, match="identity mismatch"):
        load_exp106_crossing(payload, foreign_config)

...
from data.expander_code.exp106.exp106_pipeline import EXPERIMENT_ID
from data.expander_code.exp106.exp106_pipeline import replay as replay_module
from data.expander_code.exp106.exp106_pipeline import worker as worker_module
...
from data.expander_code.exp106.exp106_pipeline.crossing import (
    classify_crossing,
    cluster_bootstrap,
    crossing_location,
    wilson_interval,
...
from data.expander_code.exp106.exp106_pipeline.io import arrays_sha256, canonical_json


CONFIG_PATH = Path(os.environ.get(
    "EXP106_TEST_CONFIG_PATH",
    REPO_ROOT / "data/expander_code/exp106/config/noisy_mc.pilot.v1.json",
))
PILOT_CONFIG_PATH = (
    REPO_ROOT / "data/expander_code/exp106/config/noisy_mc.pilot.v1.json"
)
PRODUCTION_CONFIG_PATH = (
    REPO_ROOT / "data/expander_code/exp106/config/noisy_mc.v1.json"
)
REMOTE_CONFIG_PATH = (
    REPO_ROOT / "data/expander_code/exp106/config/noisy_mc.remote.v1.json"
)
...
def complete_aggregate_factory(pilot_config, pilot_registry):
    ...
    runs to tens of thousands of codes: a 20,000-replicate cluster bootstrap
    over it would draw billions of indices, which is a production run, not a
    unit test. The pilot's two sizes are exactly the primary pair, so the
    crossing classifier is still exercised on the shape it will actually decide.
    """
    ...
    arrays.update({
        "schema_version": "exp106.aggregate.v1",
...
def foreign_config(pilot_config):
    ...
    """
    from data.expander_code.exp106.exp106_pipeline.config import ensure_config

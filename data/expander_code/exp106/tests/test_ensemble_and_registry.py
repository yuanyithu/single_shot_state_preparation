...
from data.expander_code.exp106.exp106_pipeline.model import load_model
...
def test_registry_hash_binds_the_whole_file(tmp_path, registry):
    ...
    from data.expander_code.exp106.exp106_pipeline.ensemble import REGISTRY_COLUMNS
    from data.expander_code.exp106.exp106_pipeline.io import atomic_npz, canonical_json
...
def test_registry_hash_binds_the_metadata_too(tmp_path, registry):
    ...
    metadata["seed_namespace"] = "exp106.tampered"
    path = tmp_path / "registry.npz"
    atomic_npz(path, dict(
...
def test_census_reports_a_normalized_distance_distribution():
    report = census(3, 120, "ab" * 32, "exp106.test.census")

"""exp106 and exp104 must be the same function at q = 0.

exp106 adds a branch to a machine exp104 already certified. The dangerous part
...
def test_models_are_byte_identical(exp104_registry_rows, m, index):
    ...
    exp104_model, exp106_model = build_paired_models(row)
    assert np.array_equal(exp104_model.H_Z, exp106_model.H_Z)
    assert np.array_equal(exp104_model.H_X, exp106_model.H_X)
    assert np.array_equal(exp104_model.logical_Z, exp106_model.logical_Z)
    assert exp104_model.n == exp106_model.n == 25 * m ** 2
    assert exp104_model.k == exp106_model.k == m ** 2
    assert exp106_model.n_checks == 12 * m ** 2
    assert exp104_model.classical_H_sha256 == exp106_model.classical_H_sha256
...
def test_q0_verdicts_and_labels_agree(exp104_registry_rows, m, index):
    ...
    for p in P_TOKENS:
        result = compare_q0_trials(
            exp104_model, exp106_model, p, seed=7919 + index, trials=TRIALS,
        )
...
def test_the_comparison_would_notice_a_difference(exp104_registry_rows):
    ...
    exp104_model, exp106_model = build_paired_models(row)
    tampered = np.array(exp106_model.H_Z, copy=True)
    tampered[0, 0] ^= 1
    object.__setattr__(exp106_model, "H_Z", tampered)
    with pytest.raises(ValueError, match="models disagree on H_Z"):
        compare_q0_trials(exp104_model, exp106_model, "0.06", seed=1, trials=2)


def test_row_rekeying_preserves_every_other_field(exp104_registry_rows):
    row = exp104_registry_rows["m03_c00000"]
    converted = exp104_row_to_exp106(row)

...
Every gate here is about the production plan, so the whole module is inert until
Validation 003 freezes it. It is a skip rather than a collection error on
purpose: an unimportable module would take the rest of the suite down with it,
and the skip is itself load-bearing -- nd-3 qualification requires zero skipped
tests, so an unfrozen plan cannot reach the machine.
"""
...
pytestmark = pytest.mark.skipif(
    not PRODUCTION_PLAN_FROZEN,
    reason="the production plan is frozen by Validation 003",
)

TASKS_PER_M = tasks_per_m(CODES_PER_M, CODES_PER_TASK) if PRODUCTION_PLAN_FROZEN else {}
TOTAL_CODES = sum(CODES_PER_M.values()) if PRODUCTION_PLAN_FROZEN else 0

from data.expander_code.exp106.exp106_pipeline.preflight import estimate_resources
from data.expander_code.exp106.exp106_pipeline.raw import raw_filename
from data.expander_code.exp106.exp106_pipeline.replay import (
...
def test_committed_replay_subsample_is_deterministic(remote_config):
    first = committed_replay_blocks(remote_config)
    second = committed_replay_blocks(remote_config)
    assert first == second


def test_committed_replay_subsample_is_about_ten_percent(remote_config):
...
def test_committed_replay_covers_every_m_and_therefore_every_p(remote_config):
    keys = expected_replay_keys(remote_config)
    assert {m for m, _ in keys} == set(M_VALUES)
    # Each task spans the whole p grid, so covering every m covers every cell type.
    assert len(keys) == sum(
        len(value) for value in committed_replay_blocks(remote_config).values()
    )


def test_replay_subsample_depends_on_the_frozen_seed(remote_config):
    shifted = copy.deepcopy(remote_config)
...
def test_replay_report_validation_requires_full_coverage(remote_config):
    from data.expander_code.exp106.exp106_pipeline.replay import (
        _results_manifest_sha256,
        REPLAY_SCHEMA,
    )
    from data.expander_code.exp106.exp106_pipeline.config import code_id
    from data.expander_code.exp106.exp106_pipeline.seeds import derive_seed
    from data.expander_code.exp106.exp106_pipeline.config import block_code_indices

    keys = sorted(expected_replay_keys(remote_config))
    results = []
    for m, block in keys:
        indices = block_code_indices(remote_config, m, block)
        results.append({
            "status": "PASS", "reason": "", "m": m, "block_index": block,
            "codes": len(indices),
            "trials": (
                len(indices) * len(remote_config["p_tokens"])
                * int(remote_config["trials_per_code_p"][str(m)])
            ),
            "replay_control_seed": derive_seed(
                remote_config, "replay", code_id(m, indices[0]),
                remote_config["p_tokens"][0], block,
            ),
            "raw_sha256": "a" * 64,
    ...
    report = {
        ...
        "device_name": remote_config["environment"]["device_name"],
    ...
    assert validate_replay_report(report, remote_config) is report

    dropped = copy.deepcopy(report)
    dropped["results"] = dropped["results"][:-1]
    dropped["tasks"] -= 1
    with pytest.raises(ValueError):
        validate_replay_report(dropped, remote_config)

    relabelled = copy.deepcopy(report)
    relabelled["status"] = "INVALID"
    with pytest.raises(ValueError):
        validate_replay_report(relabelled, remote_config)
...
def test_resource_estimate_passes_under_the_frozen_caps(remote_config):
    ...
    assert estimate["total_codes"] == TOTAL_CODES
    assert estimate["total_tasks"] == sum(TASKS_PER_M[m] for m in M_VALUES)
    assert estimate["total_trials"] == sum(
        CODES_PER_M[m] * len(remote_config["p_tokens"])
        * int(remote_config["trials_per_code_p"][str(m)])
        for m in M_VALUES
    )
...
def test_run_root_must_sit_under_the_frozen_run_root(remote_config, tmp_path):
    resolved = remote_cli.resolve_remote_run_root(
        "~/.single_shot/runs/exp106_noisy_v1_001", remote_config,
    )
    assert resolved.name == "exp106_noisy_v1_001"
    with pytest.raises(ValueError):
...
def test_remote_commands_refuse_the_local_config(production_config, pilot_config):
    assert production_config["schema_version"] == "exp106.config.v1"
    assert pilot_config["schema_version"] == "exp106.config.pilot.v1"
    for config in (production_config, pilot_config):
        with pytest.raises(ValueError):
            remote_cli._require_remote_config(config)
...
def test_incomplete_raw_evidence_is_refused_when_completeness_is_required(
    ...
    raw_root = tmp_path / "raw"
    raw_root.mkdir()
    (raw_root / raw_filename(remote_config, 8, 0)).write_bytes(b"")
...
def test_both_preflight_entry_points_benchmark_the_same_grid_points(remote_config):
    ...
    exp106 replaced exp104's hard-coded PREFLIGHT["p_tokens"] with a positional
    ...
    """
    from data.expander_code.exp106.exp106_pipeline.preflight import (
        benchmark_p_tokens,
    )

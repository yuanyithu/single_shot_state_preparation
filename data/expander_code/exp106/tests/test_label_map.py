...
residual pairing against logical_Z is not the logical class any more. exp106
...
from data.expander_code.exp106.exp106_pipeline.audit_scorer import (
    apply_label_map,
...
def test_label_basis_matches_exp101_frame(model):
    ...
    from data.expander_code.exp106.exp106_pipeline.exp101_bridge import load_exp101
...
def test_the_failure_criterion_differs_from_exp104_at_nonzero_syndrome(model):
    """Where exp106 genuinely parts company with exp104.
    ...
    calls every such residual a failure. exp106 scores it by its class alone,
    ...
    "simplify" exp106 back to exp104's scoring.
    """
    from data.expander_code.exp106.exp106_pipeline.worker import score_logical_class
    ...
    for _ in range(64):
        ...
        exp106_failed = bool(logical_label(model, residual).any())
        disagreements += int(exp104_failed != exp106_failed)
    assert disagreements > 0, (
        "expected residuals with nonzero syndrome but trivial logical class, "
        "which exp104 counts as failures and exp106 does not"
    )

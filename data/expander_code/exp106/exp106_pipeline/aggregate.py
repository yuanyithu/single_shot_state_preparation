"""Fail-closed aggregation of the exp106 ensemble scan.
...
def panel_layout(config):
    ...
    Panels are unequal across m in exp106, so every array that is indexed by a
...
def _ingest_task(arrays, raw, config, errors):
    ...
    if raw["schema_version"] != "exp106.raw.v1" or raw["experiment_id"] != EXPERIMENT_ID:

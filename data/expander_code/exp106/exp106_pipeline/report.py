"""Publication artifacts for exp106: curves, contrasts, strata, plots, report.

This is exp105's corrected report generator, adopted here as the in-package
module. exp105 could not take these fixes: `exp105_pipeline/report.py` is bound
by `source_tree_sha256` to the freeze that produced its published aggregate, so
the corrections had to live in a separate `publication/` package. exp106 has no
aggregate yet, which is exactly when the fixes are free.

What was wrong, and is fixed here: `write_contrasts` referenced an undefined
`ms` and killed the *remote* aggregate stage after the NPZ had already been
written; `write_code_diagnostics` read a `syndrome_mismatch_rate` field that the
q>0 aggregate does not have; and `%.2f` formatting of `p` collapsed distinct
grid points onto the same string."""
...
def p_axis_scale(p_values):
    """Log x axis only when the frozen grid actually spans a decade.

    exp105's grid ran from 0.001 to 0.07 and a log axis was the only readable
    choice. exp106's grid rule can instead return ten uniformly spaced points
    inside a narrow bracket, where a log axis compresses exactly the region the
    experiment exists to show. The grid is frozen before any task runs, so this
    depends on the plan and never on the measurement.
    """
    finite = np.asarray(p_values, dtype=float)
    finite = finite[np.isfinite(finite) & (finite > 0.0)]
    if finite.size == 0:
        return "linear"
    return "log" if finite.max() / finite.min() >= 10.0 else "linear"
...
def write_report(directory, payload, config):
    ...
    report = {
        "schema_version": "exp106.report.v1",

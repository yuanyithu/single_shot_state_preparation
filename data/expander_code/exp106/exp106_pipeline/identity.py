...
def runtime_identity(config, verify_source=False, repo_root=None):
    ...
    if config["schema_version"] in REMOTE_SCHEMAS:
    ...
    if (
        config["schema_version"] in REMOTE_SCHEMAS
    ...
    if not decoder_binary_path().name.endswith(config["decoder_binary"]["filename_suffix"]):
        raise ValueError("runtime identity mismatch for decoder binary filename")
    if verify_source:
        ...
        if diff.returncode != 0 or dirty:
            raise ValueError("exp106 source differs from the frozen source commit")
    return actual


def require_canonical_python():
    if sys.version_info[:3] != (3, 12, 12):
        raise ValueError("exp106 formal statistics require Python 3.12.12")
...
def _require_file_matches_commit(root, path, commit):
    ...
    if path.read_bytes() != committed:
        raise ValueError(f"formal exp106 artifact differs from source commit: {relative}")
...
def verify_frozen_repository(config_path, repo_root=None):
    ...
    if dirty:
        raise ValueError("formal exp106 contract/config/source has uncommitted changes")
    subprocess.run(
    ...
    if pushed.returncode != 0:
        raise ValueError("formal exp106 HEAD is not present in the configured upstream")
...
def verify_remote_deployment(
    ...
    config = ensure_config(config)
    # Both remote phases: the production config for the scan, and the pilot
    # remote config for the outcome-blind cost benchmark that has to run on
    # nd-3 before the production plan exists.
    if config["schema_version"] not in REMOTE_SCHEMAS:
        raise ValueError("remote deployment requires a remote config schema")

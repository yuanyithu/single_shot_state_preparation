...
def _time_raw_io(payload):
    with tempfile.TemporaryDirectory(prefix="exp106-preflight-") as temporary:
...
def benchmark_task(m, code_index, p_token, config, registry_rows, trials=None,
    ...
    `time_io=False` skips the synthetic raw round trip, which needs the
    frozen per-m block size and trial count. The cost benchmark runs before
    those exist and only wants `kappa_m` and `c_m`.
    """
    ...
    io_timings = (
        _time_raw_io(_synthetic_raw(m, config, identity)) if time_io else {}
    )
    clear_model_cache()
    return {
        "m": int(m),
...
def run_local_preflight(config, registry_rows):
    ...
    return {
        "schema_version": "exp106.local_resource_preflight.v1",
        "experiment_id": config["experiment_id"],
        "config_sha256": config["config_sha256"],
        "registry_sha256": config["registry_sha256"],

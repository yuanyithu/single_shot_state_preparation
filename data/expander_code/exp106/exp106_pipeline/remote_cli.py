...
exp106 has no stage split: there is no preliminary curve to authorize a second
...
REMOTE_PREFLIGHT_SCHEMA = "exp106.remote_resource_preflight.v1"
REMOTE_QUALIFICATION_SCHEMA = "exp106.remote_environment_qualification.v1"
REMOTE_SCAN_SCHEMA = "exp106.remote_scan.v1"
...
COMMITTED_GATE_DIRECTORY = "004_remote_gate_20260812"
COMMITTED_QUALIFICATION_RELATIVE = Path(
    f"data/expander_code/exp106/validation/{COMMITTED_GATE_DIRECTORY}/"
...
COMMITTED_COST_BENCHMARK_RELATIVE = Path(
    "data/expander_code/exp106/validation/003_locating_pilot_20260812/"
    "cost_benchmark.json"
)
QUALIFICATION_GROUPS = (
    ("exp106", ("data/expander_code/exp106/tests",)),
    ...
    # exp105's suite minus `test_sector_ti_fast.py`. That file is Track B, the
    # numba sector-TI fast path, which exp106 never uses and whose absence from
    # the shipped archive is deliberate: it would drag `exp105/anchor` and a
    # numba compile into a gate that exists only to check the package exp106's
    # equality gate decodes against. exp105 is closed and frozen, so naming its
    # files is stable rather than brittle.
    ("exp105", (
        "data/expander_code/exp105/tests/test_aggregate_fail_closed.py",
        "data/expander_code/exp105/tests/test_contract_and_config.py",
        "data/expander_code/exp105/tests/test_crossing_decisions.py",
        "data/expander_code/exp105/tests/test_decoder_determinism.py",
        "data/expander_code/exp105/tests/test_ensemble_and_registry.py",
        "data/expander_code/exp105/tests/test_exp104_equality.py",
        "data/expander_code/exp105/tests/test_label_map.py",
        "data/expander_code/exp105/tests/test_publication_loader.py",
        "data/expander_code/exp105/tests/test_remote_execution.py",
        "data/expander_code/exp105/tests/test_worker_and_replay.py",
    )),
    ("exp102", (
...
# exp104 and exp105 are included because exp106's two equality gates decode
# against their packages: exp104 supplies the q = 0 path and exp105 the q > 0
# augmented path. A change in either would silently change what exp106 means,
# and the exact pass counts make that a gate rather than a hope. The exp101 and
# exp102 groups are the same certified subsets exp103 and exp104 qualified
# against.
#
...
def _require_pilot_remote_config(config):
    """The cost benchmark is the one nd-3 command that runs before the freeze."""
    if config["schema_version"] != PILOT_REMOTE_CONFIG_SCHEMA:
        raise ValueError("cost-benchmark requires the frozen pilot remote config")
    return config
...
def _resolve_pilot_run_root(run_root, config):
    """Same confinement as the production runs, for the pilot remote config."""
    config = _require_pilot_remote_config(config)
    allowed = Path(config["execution_profile"]["run_root"]).expanduser().resolve()
    given = Path(run_root).expanduser()
    resolved = (allowed / given.name) if len(given.parts) == 1 else given.resolve()
    if resolved.parent != allowed:
...
def require_expected_pass_counts():
    """Refuse to qualify against pass counts nobody has measured yet."""
    missing = sorted(
        name for name, count in QUALIFICATION_EXPECTED_PASSES.items() if count is None
    )
    if missing:
        raise ValueError(
            "qualification pass counts are unset for "
            f"{', '.join(missing)}; they must be measured on the frozen plan "
            "before any remote gate runs"
        )


def run_environment_qualification(config, source_root):
    config = _require_remote_config(config)
    require_expected_pass_counts()
    ...
    environment.update({
        ...
        # exp104's and exp105's suites are part of this qualification, because
        # exp106's two equality gates decode against their packages. Their
        # frozen-config fixtures have to be pointed at the *remote* configs as
        # well; left at their defaults they would check macmini identities on
        # nd-3 and fail for a reason that has nothing to do with the environment.
        # exp105's first nd-3 qualification failed on exactly this.
        ...
        "EXP105_TEST_CONFIG_PATH": str(
            source_root
            / "data/expander_code/exp105/config/noisy_mc.remote.v1.json"
        ),
        "EXP106_TEST_CONFIG_PATH": str(
            source_root / "data/expander_code/exp106/config/noisy_mc.remote.v1.json"
        ),
    })
...
def run_remote_cost_benchmark(config, registry_rows):
    """Measure kappa_m and c_m on nd-3, with no dependence on the frozen plan.

    This exists to break a circularity exp105 could not: its only path to nd-3
    costs was `preflight`, which already requires the production plan, and the
    allocation rule that produces that plan requires the costs. exp105 resolved
    it by guessing with macmini numbers, and the resource gate blocked at 5,368
    core-hours against a cap of 800.

    Outcome-blind by construction: `benchmark_task` times the decode and its
    replay, compares them, and discards the comparison. Nothing about which
    trials failed is returned, stored or reachable from here.
    """
    config = _require_pilot_remote_config(config)
    spec = config["preflight"]
    tokens = benchmark_p_tokens(config)
    tasks = [
        benchmark_task(
            m, code_index, token, config, registry_rows,
            trials=COST_BENCHMARK_TRIALS, time_io=False,
        )
        for m in M_VALUES
        for code_index in spec["code_indices"]
        for token in tokens
    ]
    # Upper bounds, matching what the resource projection will later assume:
    # the maximum over the benchmarked grid points at each m.
    kappa = _upper_by_m(tasks, "model_seconds", M_VALUES)
    per_trial = _upper_by_m(tasks, "measurement_seconds_per_trial", M_VALUES)
    core = {
        ...
        "outcome_blind": True,
        "benchmark_p_tokens": tokens,
        "code_indices": list(spec["code_indices"]),
        "trials_per_batch": COST_BENCHMARK_TRIALS,
        "seed_namespace": config["namespaces"]["benchmark"],
        "kappa_seconds_upper": {str(m): float(kappa[m]) for m in M_VALUES},
        "c_seconds_upper": {str(m): float(per_trial[m]) for m in M_VALUES},
        "peak_rss_gib": max(task["peak_rss_gib"] for task in tasks),
        "tasks": tasks,
    }
    return dict(core, report_sha256=sha256_json(core))
...
def build_parser():
    parser = argparse.ArgumentParser(description="exp106 remote execution on nd-3")
    sub = parser.add_subparsers(dest="command", required=True)
    for name in (
        "cost-benchmark", "qualify", "preflight", "scan", "replay", "aggregate",
    ):
...
def main(argv=None):
    ...
    if args.name == "cost-benchmark":
        # Runs before the production plan exists, so it uses the pilot
        # remote config and its own run root confinement.
        run_root = _resolve_pilot_run_root(args.run_root, config)
        ...
        report = run_remote_cost_benchmark(config, rows)
        path = run_root / "validation" / "cost_benchmark.json"
        if path.exists():
            raise FileExistsError(f"cost benchmark is immutable: {path}")
        atomic_json(path, report)
        print("MEASURED", report["device"], json.dumps({
            "kappa_seconds_upper": report["kappa_seconds_upper"],
            "c_seconds_upper": report["c_seconds_upper"],
        }, sort_keys=True), sha256_file(path))
        return 0

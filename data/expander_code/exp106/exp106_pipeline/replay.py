...
exp106 replays a preregistered ten percent of tasks rather than all of them.
...
REPLAY_SCHEMA = "exp106.replay.v1"
RESULT_FIELDS = {
    "status", "reason", "m", "block_index", "codes", "trials",
    "replay_control_seed", "raw_sha256", "error_stream_sha256",
    "readout_stream_sha256", "correction_stream_sha256", "label_stream_sha256",
}
REPORT_FIELDS = {
...
def committed_replay_blocks(config):
    ...
    if policy["policy"] != "committed_random_subsample":
        raise ValueError("unexpected exp106 replay policy")
    ...
    per_m_tasks = tasks_per_m(counts, sizes)
    selected = {}
    for m in config["m_values"]:
        total = per_m_tasks[int(m)]
...
def replay_task(raw_or_path, config, registry_rows):
    ...
    per_m_tasks = tasks_per_m(counts, sizes)
    if m not in config["m_values"] or not 0 <= block_index < per_m_tasks[m]:
    ...
    trials = int(config["trials_per_code_p"][str(m)])
    ...
    expected_arrays = {
        ...
        "readout_match": ((codes, len(tokens), trials), np.dtype(np.bool_)),
    ...
    try:
        for slot, index in enumerate(indices):
            ...
            audit_label_map = independent_label_map(model.H_Z, model.logical_Z)
            if (
            ...
            for p_slot, token in enumerate(tokens):
                ...
                for trial in range(trials):
                    ...
                    syndrome = np.bitwise_xor(
                        np.asarray(model.H_Z @ error, dtype=np.uint8) & np.uint8(1),
                        readout,
                    )
                    ...
                    failed = logical_class_score(generators, error, correction, model.n)
                    matched = bool(np.array_equal(correction[model.n:], readout))
                    residual = np.bitwise_xor(error, correction[:model.n])
                    labels = apply_label_map(audit_label_map, residual)
                    if (
                        failed != bool(raw["failure_flags"][slot, p_slot, trial])
                        or failed != bool(labels.any())
                        or not np.array_equal(
                            labels, raw["logical_labels"][slot, p_slot, trial]
                        )
                        or matched != bool(raw["readout_match"][slot, p_slot, trial])
                    ...
                    label_digest.update(np.asarray(labels, dtype=np.uint8).tobytes())
    finally:
        clear_model_cache()

    hashes = {
        "error_stream_sha256": error_digest.hexdigest(),
        "readout_stream_sha256": readout_digest.hexdigest(),

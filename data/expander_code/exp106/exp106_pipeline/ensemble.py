...
exp106 differs from exp104 in two mechanical ways. Panels are unequal across m,
...
12,000-row JSON is already 3.3 MB, and exp106 holds of order 10^5 rows.
"""
...
def load_registry(path, verify_seeds=False):
    ...
    if metadata["schema_version"] != REGISTRY_SCHEMA:
        raise ValueError("unexpected exp106 registry schema")
    if metadata["columns"] != list(REGISTRY_COLUMNS):
        raise ValueError("exp106 registry column order is not the frozen order")
    if _registry_digest(metadata, columns) != claimed:
        raise ValueError("exp106 registry SHA256 mismatch")
    ...
    if ids != expected:
        raise ValueError("exp106 registry code IDs are incomplete, reordered or duplicated")
    registry = {
        "metadata": metadata,
        "columns": columns,
...
def run_census(output_path, accepted_target, master_seed_hex=MASTER_SEED_HEX,
    ...
    if m_values is None:
        m_values = sorted(set(CENSUS_ONLY_M_VALUES) | set(M_VALUES))
    per_m = {}
    for m in m_values:
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="exp106 ensemble construction")

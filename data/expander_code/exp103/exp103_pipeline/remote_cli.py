...
from .identity import runtime_identity, verify_remote_deployment
...
def resolve_remote_run_root(run_root, config):
    ...
    if requested.exists() and requested.is_symlink():
        raise ValueError("remote run root must not be a symlink")
    return resolved
...
def _bytecode_clean(source_root):
    ...
    return not any(
        path.name == "__pycache__" or (path.is_file() and path.suffix == ".pyc")
...
def run_environment_qualification(config, deployment, source_root):
    ...
    for name, argv in _qualification_argv():
        ...
        matches = re.findall(rb"(?:^|\s)(\d+) passed(?:\s|,|$)", stdout + b"\n" + stderr)
...
def validate_environment_qualification(report, config):
    ...
    required = {
        ...
        "conda_environment", "python_executable", "python_no_bytecode_flag",
        "pythondontwritebytecode", "pytest_cache_disabled",
        "bytecode_clean_before", "bytecode_clean_after", "deployment", "groups",
        "total_passed",
    }

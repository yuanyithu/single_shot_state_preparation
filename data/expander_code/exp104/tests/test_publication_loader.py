...
from data.expander_code.exp104.exp104_pipeline.aggregate import ARRAY_FIELDS, SCALAR_FIELDS
from data.expander_code.exp104.exp104_pipeline.crossing import CERTIFIED
...
def test_loader_accepts_the_complete_aggregate(complete_aggregate_factory, frozen_config):
    ...
    accepted = load_exp104_crossing(payload, frozen_config)
...
def test_loader_round_trips_through_an_npz_file(
    ...
    accepted = load_exp104_crossing(path, frozen_config)
...
def test_loader_rejects_a_tampered_payload_without_rehashing(
    ...
    with pytest.raises(ValueError, match="payload SHA256"):
        load_exp104_crossing(payload, frozen_config)
...
def test_loader_rejects_a_relabelled_terminal_status(
    ...
    payload = complete_aggregate_factory()
    payload["terminal_status"] = "EXP104_NO_CERTIFIED_CROSSING"
    rehash_aggregate(payload)

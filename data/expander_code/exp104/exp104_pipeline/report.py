"""Publication artifacts for exp104: curves, contrasts, strata, plots, report."""
...
from .config import CODES_PER_M, M_VALUES, ensure_config
...
def write_distance_strata(path, payload):
    ...
    for m_index, m in enumerate(M_VALUES):
        for d_index, distance in enumerate(DISTANCE_STRATA):
            ...
            for p_index, p in enumerate(payload["p_values"]):
                rows.append([
                    m, distance, codes, f"{codes / CODES_PER_M:.6f}", f"{p:.2f}",
...
def write_composition(path, payload):
    ...
    for m_index, m in enumerate(M_VALUES):
        for d_index, distance in enumerate(DISTANCE_STRATA):
            codes = int(payload["strata_code_counts"][m_index, d_index])
            if codes:
                rows.append([m, distance, codes, f"{codes / CODES_PER_M:.6f}"])
...
def write_plots(directory, payload):
    ...
    axes[0].set_title(
        f"{DECODER_LABEL} ensemble mean, {CODES_PER_M} random codes per m"
    )
    axes[0].legend()
    ...
    width = 0.13
    for d_index, distance in enumerate(DISTANCE_STRATA):
        fractions = payload["strata_code_counts"][:, d_index] / CODES_PER_M
        if not fractions.any():
            continue
        axes[1].bar(
            np.arange(len(M_VALUES)) + d_index * width, fractions, width,
            label=f"d={distance}",
        )
    axes[1].set_xticks(np.arange(len(M_VALUES)) + 2 * width)
    axes[1].set_xticklabels([str(m) for m in M_VALUES])
    axes[1].set_xlabel("m")
...
def write_report(directory, payload, config):
    ...
    lines = [
        ...
        f"- Ensemble: {CODES_PER_M} randomly generated codes per m, "
        f"m = {M_VALUES[0]}..{M_VALUES[-1]}, no post-selection.",
        f"- {int(config['trials_per_code_p'])} trials per code and p over "
    ...
    report = {
        "schema_version": "exp104.report.v1",
        ...
        "trials_per_code_p": int(payload["trials_per_code_p"]),

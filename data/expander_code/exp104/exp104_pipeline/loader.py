"""Independent re-verification of a published exp104 aggregate.
...
from .aggregate import ARRAY_FIELDS, DISTANCE_STRATA, SCALAR_FIELDS
...
def _load_npz(path):
    with np.load(path, allow_pickle=False) as data:
        ...
        if missing or extra:
            raise ValueError("aggregate NPZ fields do not match exp104.aggregate.v1")
        payload = {}
...
def load_exp104_crossing(path_or_payload, config, require_complete=True):
    ...
    for field, expected in (
        ...
        ("trials_per_code_p", int(config["trials_per_code_p"])),
    ):
    ...
    if not np.all(payload["trial_counts"][status == "REPORTABLE"] == trials):
    ...
    with np.errstate(invalid="ignore"):
        expected_rates = counts / float(trials)
    ...
    for index in sample:
        low, high = wilson_interval(int(flat_counts[index]), trials)
    ...
    for m_index in range(len(M_VALUES)):
        ...
        for p_index in range(n_p):
            ...
            trials_total = CODES_PER_M * trials
            ...
            if abs(
                payload["cluster_se"][m_index, p_index] - std / np.sqrt(CODES_PER_M)
            ) > 1e-9:

...
exp104 has no stage split: there is no preliminary curve to authorize a second
...
REMOTE_PREFLIGHT_SCHEMA = "exp104.remote_resource_preflight.v1"
REMOTE_QUALIFICATION_SCHEMA = "exp104.remote_environment_qualification.v1"
REMOTE_SCAN_SCHEMA = "exp104.remote_scan.v1"
...
COMMITTED_QUALIFICATION_RELATIVE = Path(
    "data/expander_code/exp104/validation/003_remote_gate_20260808/"
...
QUALIFICATION_EXPECTED_PASSES = {"exp104": 131, "exp101": 58, "exp102": 17}
...
def run_environment_qualification(config, source_root):
    ...
    environment.update({
        ...
        "EXP104_TEST_CONFIG_PATH": str(
            source_root / "data/expander_code/exp104/config/ensemble_mc.remote.v1.json"
        ),
    })
...
def validate_remote_resource_preflight(report, config):
    ...
    blocks = committed_replay_blocks(config)
    if report["committed_replay_blocks"] != {str(m): blocks[m] for m in M_VALUES}:
...
def _run_one_task(payload):
    ...
    output = Path(raw_root) / raw_filename(m, block_index)
...
def _planned_tasks(config):
    return [(m, block) for m in M_VALUES for block in range(TASKS_PER_M[m])]
...
def _assert_no_unplanned_npz(raw_root, config, require_complete=False):
    ...
    expected = {
        raw_root / raw_filename(m, block) for m, block in _planned_tasks(config)
    }
...
def run_remote_replay(config_path, run_root, num_workers, deployment_root,
    ...
    payloads = [
        (str(raw_root / raw_filename(m, block)), str(config_path)) for m, block in keys
    ]
    results = []
...
def build_parser():
    parser = argparse.ArgumentParser(description="exp104 remote execution on nd-3")

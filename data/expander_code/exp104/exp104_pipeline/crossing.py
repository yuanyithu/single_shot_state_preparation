...
the nine grid points of the primary contrast. exp103 took a max-absolute
deviation simultaneously over six curves, thirteen points and five adjacent
contrasts, which is what produced a half-width of 0.2601 and made certification
impossible regardless of the point estimates.
...
sample can do. exp104 instead certifies a negative point and a later positive
...
CERTIFIED = "EXP104_CERTIFIED_CROSSING"
NOT_CERTIFIED = "EXP104_NO_CERTIFIED_CROSSING"
...
def _resampled_means(failures, trials_per_code, rng, replicates, chunk=500):
    ...
    the interval covers ensemble variation rather than only shot noise; with
    four trials per code-p the two are combined automatically because a resampled
...
def cluster_bootstrap(failures_by_m, trials_per_code, config, label):
    ...
    for m_index, failures in enumerate(failures_by_m):
        curves[m_index] = _resampled_means(failures, trials_per_code, rng, replicates)
        point[m_index] = failures.sum(axis=0) / float(
            failures.shape[0] * trials_per_code
        )

"""Local command line for exp104: config authoring, tasks, replay, aggregation."""
...
def _base_config(registry_sha256, source_commit, tree_sha256):
    return {
        "schema_version": CONFIG_SCHEMA,
        "experiment_id": EXPERIMENT_ID,
        "objective": "bposd_ensemble_block_logical_failure_crossing_q0",
        ...
        "p_tokens": P_TOKENS,
        "trials_per_code_p": TRIALS_PER_CODE_P,
...
def _remote_config(base):
    config = dict(base)
    config.update({
        "schema_version": REMOTE_CONFIG_SCHEMA,
        "environment": {
            "device_name": "nd-3",
            "hostname": "nd-3",
...
def command_write_config(args):
    ...
    base = _base_config(registry["registry_sha256"], args.source_commit, tree)
    config_dir = Path(__file__).resolve().parents[1] / "config"
    atomic_json(config_dir / "ensemble_mc.v1.json", base)
    print("local ", sha256_json(base))
    if args.remote:
        remote = _remote_config(base)
        atomic_json(config_dir / "ensemble_mc.remote.v1.json", remote)
...
def command_run_task(args):
    ...
    path = Path(args.output) / raw_filename(args.m, args.block)
...
def build_parser():
    parser = argparse.ArgumentParser(description="exp104 local pipeline")

...
- [`exp21_q050_heavy_p018_022_combined_summary`](with_measurement_noise/exp21_q050_heavy_p018_022_combined_summary/)：`q=0.0500` 池化综合分析；`288` disorder 后 `L3-L4` crossing 约 `p≈0.193`，`L4-L5` 到 `p=0.22` 仍未 crossing；后续判断已被 `exp25` 的 `1152` disorder dense grid 更新。
...
- [`exp24a_q001_q050_q100_p018_022_dense_20260426_nd1`](with_measurement_noise/exp24a_q001_q050_q100_p018_022_dense_20260426_nd1/) / [`exp24b_q001_q050_q100_p018_022_dense_20260426_nd2`](with_measurement_noise/exp24b_q001_q050_q100_p018_022_dense_20260426_nd2/) / [`exp24c_q001_q050_q100_p018_022_dense_20260426_nd3`](with_measurement_noise/exp24c_q001_q050_q100_p018_022_dense_20260426_nd3/)：`q=0.0100/0.0500/0.1000` 在 `p=0.180~0.220`、步长 `0.005` 的三节点独立高采样 run，各 q 每节点 `384` disorder。
- [`exp25_q001_q050_q100_p018_022_dense_combined_summary`](with_measurement_noise/exp25_q001_q050_q100_p018_022_dense_combined_summary/)：`exp24a/b/c` 池化综合分析；每个 q 为 `1152` disorder。`q=0.0500` 给出共同 crossing window `p≈0.2032~0.2121`，`q=0.0100` 到 `0.220` 仍偏左侧，`q=0.1000` 在 `0.180` 已偏右侧；需注意 `q=0.0100/0.0500` mixing 诊断仍未通过。
- [`exp26a_fixed_p010_q000_100_20260426_nd1`](with_measurement_noise/exp26a_fixed_p010_q000_100_20260426_nd1/) / [`exp26b_fixed_p010_q000_100_20260426_nd2`](with_measurement_noise/exp26b_fixed_p010_q000_100_20260426_nd2/) / [`exp26c_fixed_p010_q000_100_20260426_nd3`](with_measurement_noise/exp26c_fixed_p010_q000_100_20260426_nd3/)：固定 `p=0.1000`、扫描 `q=0.0000~0.1000` 步长 `0.0100` 的三节点独立 run，各 q 每节点 `512` disorder。
- [`exp27_fixed_p010_q000_100_combined_summary`](with_measurement_noise/exp27_fixed_p010_q000_100_combined_summary/)：`exp26a/b/c` 池化综合分析；每个 q 为 `1536` disorder。`L4-L5` gap 在 `q≈0.0247` 翻号，`L3-L4` gap 在 `q≈0.0608` 翻号，两者不重合，因此固定 `p=0.1` 下暂不宣称共同 q-threshold。
- [`exp28a_fixed_p010_q000_100_L6_20260427_nd1`](with_measurement_noise/exp28a_fixed_p010_q000_100_L6_20260427_nd1/) / [`exp28b_fixed_p010_q000_100_L6_20260427_nd2`](with_measurement_noise/exp28b_fixed_p010_q000_100_L6_20260427_nd2/) / [`exp28c_fixed_p010_q000_100_L6_20260427_nd3`](with_measurement_noise/exp28c_fixed_p010_q000_100_L6_20260427_nd3/)：固定 `p=0.1000`、扫描 `q=0.0000~0.1000` 的 L=6 extension，各 q 每节点 `512` disorder。
- [`exp29_fixed_p010_q000_100_L3456_combined_summary`](with_measurement_noise/exp29_fixed_p010_q000_100_L3456_combined_summary/)：固定 `p=0.1000` 的 L=3/4/5/6 综合分析。L6 q>0 mixing 诊断失败、PT swap 近似为 0，因此目前只作诊断延伸，不更新最终 q-threshold 判断。
- [`exp30_cluster_runtime_compare_q050_p020_L345_local`](with_measurement_noise/exp30_cluster_runtime_compare_q050_p020_L345_local/)：本地 cluster update runtime 小基准；用于估计新 cluster 更新路径开销，不作为物理阈值结论。
- [`exp31_pq_grid_q000_050_p001_120_20260511_nd1`](with_measurement_noise/exp31_pq_grid_q000_050_p001_120_20260511_nd1/)：只用 `nd-1` 扫描 `p={0.0100,0.0300,0.0500,0.0700,0.1000,0.1200}` 与 `q=0.0000~0.0500` 步长 `0.0050`，每点 `256` disorder；完成 `3168/3168` chunks 且 `0` failed。固定 `p` 的 q 方向分析没有得到稳定共同边界，主要价值是显示 `L3-L4`/`L4-L5` finite-size drift 与 q>0 mixing 诊断问题。
- [`exp32_fixed_p050_q000_075_L34567_20260514_nd23`](with_measurement_noise/exp32_fixed_p050_q000_075_L34567_20260514_nd23/)：固定 `p=0.0500`、`q=0.0000~0.0750` 步长 `0.0050`、`L=3,4,5,6,7` 的 nd-2/nd-3 独立 seed 长扫描；每个 `(L,q)` 池化 `2048` disorder。两节点 `160/160` 子 run 完成且 `0` failed；结果显示各相邻尺寸 gap 翻号位置不一致，`q>0` convergence gate 仅 `2/75` 通过，因此不能宣称五尺寸共同 q-threshold。

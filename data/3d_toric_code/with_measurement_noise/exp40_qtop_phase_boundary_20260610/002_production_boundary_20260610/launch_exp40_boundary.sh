#!/usr/bin/env bash
#
# exp40/002 q_top phase-boundary launcher (node<->p), adapted from exp39/007.
#
# Same mechanism as exp39/007: each node runs its assigned p-cells SEQUENTIALLY
# in one screen; each cell is a FULL L=3,4,5 x q-grid `run` call (worker pool
# balances L x q x disorder across cores).
#
# exp40 changes vs exp39/007: dense centered q-grid (10 pts, step 0.004 in the
# crossing core 0.02-0.035), 9 p-values across 3 nodes, seed bases 850000+.
#
...
STAGE_DIR="$PROJECT_ROOT/data/3d_toric_code/with_measurement_noise/exp40_qtop_phase_boundary_20260610/002_production_boundary_20260610"
RUN_TIMESTAMP="${RUN_TIMESTAMP:-$(date +%Y%m%d_%H%M%S)}"
MASTER_RUN_ID="${MASTER_RUN_ID:-exp40_boundary_${RUN_TIMESTAMP}}"
...
# Common dense q grid centered on the expected crossing band 0.02-0.035.
Q_GRID="${Q_GRID:-0.008,0.012,0.016,0.020,0.024,0.028,0.032,0.038,0.046,0.058}"

# Default cell matrix: 9 p-values, 3 per node (sequential), node<->p.
# p=0.14 / 0.20 overlap exp39 (independent-seed reproduction check).
DEFAULT_CELLS=(
  "nd-1|0.01|850000|$Q_GRID"
  "nd-1|0.08|853000|$Q_GRID"
  "nd-1|0.17|856000|$Q_GRID"
  "nd-2|0.03|851000|$Q_GRID"
  "nd-2|0.11|854000|$Q_GRID"
  "nd-2|0.20|857000|$Q_GRID"
  "nd-3|0.05|852000|$Q_GRID"
  "nd-3|0.14|855000|$Q_GRID"
  "nd-3|0.22|858000|$Q_GRID"
)
if [[ -n "${CELLS:-}" ]]; then
...
# Fixed run parameters (identical to validated exp39/004/006/007 production runs).
...
log_msg "exp40 boundary host=$host workers=$workers cells=[$cells_blob]"
...
if [[ "$overall_rc" == "0" ]]; then
  ...
  log_msg "exp40 boundary host=$host ALL CELLS OK"
else
  write_json_status "$run_root/_FAILED.json" failed "$overall_rc"
  log_msg "exp40 boundary host=$host had failures rc=$overall_rc"
  exit "$overall_rc"
fi
EOF_RUNNER
  }
...
launch_host() {
  ...
  screen_name="exp40B_${RUN_TIMESTAMP}_${tag}"

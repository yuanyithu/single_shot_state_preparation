#!/usr/bin/env python3
"""exp41/003 p=0.11 L=3..7 figures (reused exp40 L3-6 + new L7).
All estimators from softmax(-delta_f); w0 sign-aware headline, q_W even-moment check."""
...
EXP41 = SD.parent
E40 = EXP41 / "../exp40_qtop_phase_boundary_20260610/004_p011_highstats_20260611"
GLOBS = [str(E40/g) for g in ("nd1","nd2","nd3","nd1_L6","nd2_L6","nd3_L6")] + \
        [str(SD/"nd1"), str(SD/"nd2")]
SIGN = np.array([[1-2*((g>>i)&1) for g in range(8)] for i in range(3)])
RNG = np.random.default_rng(20260622)
COLORS = {3:"#1f77b4",4:"#ff7f0e",5:"#2ca02c",6:"#9467bd",7:"#d62728"}
...
def main():
    ...
    axD.grid(alpha=.35); axD.legend(fontsize=9); axD.set_title("crossing FSS (L6-L7 excluded: saturated/degenerate)")

    fig.suptitle(f"exp41  p=0.11  L=3..7 × 384 disorder   |   headline w0 L3-L7 q_c={qc_hl:.4f} [{ci_hl[0]:.4f},{ci_hl[1]:.4f}]",
                 fontsize=13)
    out=SD/"p011_L34567_curves.png"; fig.savefig(out,dpi=160); plt.close(fig)
    print("wrote",out,"  headline w0 L3-L7 q_c=",round(qc_hl,4),[round(c,4) for c in ci_hl])

#!/usr/bin/env python3
"""exp41/004 p=0.21 L=3,4,5,7 figures (reused exp40/005 L3-5 + new exp41 L7).
Near p_c~0.227 collapse test. All estimators from softmax(-delta_f);
...
SD = Path(__file__).resolve().parent          # 004_.../
EXP41 = SD.parent
BASE = EXP41.parent                            # with_measurement_noise/
E40 = BASE / "exp40_qtop_phase_boundary_20260610/005_boundary_highstats_20260612"
GLOBS = [str(E40/f"nd{n}/collected/p0p21") for n in (1, 2, 3)] + \
...
def main():
    ...
    for est_name,mk,col in (("w0","o","#2ca02c"),("q_W","^","#1f77b4")):
        xs,ys,lo,hi=[],[],[],[]
        for (a,b) in pairs:
            if (a,b)==(3,4): continue   # saturation-degenerate, exclude
    ...
    axD.axhline(0.0338,color="0.4",ls=":",lw=1.2,label="p=0.11 w0 q_c (flat ref)")
    ...
    fig.suptitle(f"exp41  p=0.21 (near p_c~0.227)  L=3,4,5,7 × 384 disorder   |   "
                 f"w0 L3-L7 q_c={qc_hl:.4f} [{ci_hl[0]:.4f},{ci_hl[1]:.4f}]  — FLAT vs p=0.11 (no collapse)",
                 fontsize=12.5)
    out=SD/"p021_L34567_curves.png"; fig.savefig(out,dpi=160); plt.close(fig)

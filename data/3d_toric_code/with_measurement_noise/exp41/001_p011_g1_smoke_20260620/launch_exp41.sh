...
# exp41: STAGE_DIR overridable so one script serves G1/G2/P1 steps.
STAGE_DIR="${STAGE_DIR:-$PROJECT_ROOT/data/3d_toric_code/with_measurement_noise/exp41/001_p011_g1_smoke_20260620}"
RUN_TIMESTAMP="${RUN_TIMESTAMP:-$(date +%Y%m%d_%H%M%S)}"
MASTER_RUN_ID="${MASTER_RUN_ID:-exp41_${RUN_TIMESTAMP}}"
...
# exp41 default q grid = exp40/004 ten-point grid (enables merge with reused L3-6).
Q_GRID="${Q_GRID:-0.012,0.018,0.022,0.026,0.030,0.034,0.040,0.048,0.058,0.070}"

# exp41 default cell = G1 smoke: single node nd-1, p=0.11, single q, disorder-shard seed.
# Override with env CELLS (semicolon-separated "host|p|seed_base|q_csv") for G2/P1.
DEFAULT_CELLS=(
  "nd-1|0.11|900000|0.040"
)
if [[ -n "${CELLS:-}" ]]; then
...
FIXED_LATTICE_SIZES="${FIXED_LATTICE_SIZES:-3,4,5,6,7}"
NUM_DISORDER_SAMPLES="${NUM_DISORDER_SAMPLES:-64}"
...
launch_host() {
  ...
  screen_name="exp41_${RUN_TIMESTAMP}_${tag}"

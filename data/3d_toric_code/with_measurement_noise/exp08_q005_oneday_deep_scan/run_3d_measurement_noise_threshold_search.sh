...
cd /home/DATA1/users/yuany/.single_shot/repos/3d_toric_oneday_deep_q0p005_20260424

if command -v python3 >/dev/null 2>&1; then
  q_and_p_windows="$(python3 -c 'import base64, sys; print(base64.b64decode(sys.argv[1]).decode(), end="")' MC4wMDUwfDAuMTkwMCwwLjIwMDAsMC4yMDUwLDAuMjEwMCwwLjIxNTAsMC4yMjAwLDAuMjMwMA==)"
else
  echo "python3 is required on nd-2 to decode q/p window payload." >&2
  exit 23
fi
...
master_run_root=/home/DATA1/users/yuany/.single_shot/runs/3d_toric_oneday_deep_q0p005_20260424
...
while IFS='|' read -r syndrome_error_probability data_error_probabilities; do
  ...
  current_seed_base=$(( 2026042500 + q_index * 1000000000 ))
  ...
  echo "[launcher] starting q=$syndrome_error_probability host=nd-2 run_root=$run_root seed_base=$current_seed_base workers=$workers"
  conda run -n 11 python src/production_chunked_scan.py submit     --run-root "$run_root"     --code-family 3d_toric     --workers "$workers"     --chunk-size 3     --num-disorder-samples-total 6     --data-error-probabilities "$data_error_probabilities"     --lattice-sizes 3\,4\,5     --syndrome-error-probability "$syndrome_error_probability"     --num-burn-in-sweeps 1000     --num-sweeps-between-measurements 6     --num-measurements-per-disorder 3072     --q0-num-start-chains 8     --num-start-chains 8     --num-replicas-per-start 2     --pt-p-hot 0.44     --pt-num-temperatures 9     --pt-swap-attempt-every-num-sweeps 1     --seed-base "$current_seed_base"     --burn-in-scaling-reference-num-qubits 18     --output-stem "$output_stem"     --common-random-disorder-across-p     --git-commit-sha 0c4074557b20facc73c07c533238b8806aa6dde9

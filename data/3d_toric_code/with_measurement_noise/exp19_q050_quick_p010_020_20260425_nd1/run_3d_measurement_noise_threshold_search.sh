...
cd /home/DATA1/users/yuany/.single_shot/repos/3d_toric_exp19_q050_quick_p010_020_20260425_nd1

if command -v python3 >/dev/null 2>&1; then
  q_and_p_windows="$(python3 -c 'import base64, sys; print(base64.b64decode(sys.argv[1]).decode(), end="")' MC4wNTAwfDAuMTAwMCwwLjExMTEsMC4xMjIyLDAuMTMzMywwLjE0NDQsMC4xNTU2LDAuMTY2NywwLjE3NzgsMC4xODg5LDAuMjAwMA==)"
else
  echo "python3 is required on nd-1 to decode q/p window payload." >&2
  exit 23
fi
...
master_run_root=/home/DATA1/users/yuany/.single_shot/runs/3d_toric_exp19_q050_quick_p010_020_20260425_nd1
...
while IFS='|' read -r syndrome_error_probability data_error_probabilities; do
  ...
  current_seed_base=$(( 2026042519 + q_index * 1000000000 ))
  ...
  if [[ -n 2000 ]]; then
    burn_in_cap_args+=(
      --max-effective-num-burn-in-sweeps 2000
    )
  fi
  echo "[launcher] starting q=$syndrome_error_probability host=nd-1 run_root=$run_root seed_base=$current_seed_base workers=$workers"
  conda run -n 11 python src/production_chunked_scan.py submit     --run-root "$run_root"     --code-family 3d_toric     --workers "$workers"     --chunk-size 2     --num-disorder-samples-total 24     --data-error-probabilities "$data_error_probabilities"     --lattice-sizes 3\,4\,5     --syndrome-error-probability "$syndrome_error_probability"     --num-burn-in-sweeps 600     --num-sweeps-between-measurements 4     --num-measurements-per-disorder 768     "${submit_extra_args[@]}"     --seed-base "$current_seed_base"     --burn-in-scaling-reference-num-qubits 18     "${burn_in_cap_args[@]}"     --output-stem "$output_stem"     --common-random-disorder-across-p     --git-commit-sha 2df10e9adba79f3f1ec9e423f20554c209849bf0

...
cd /home/DATA1/users/yuany/.single_shot/repos/3d_toric_exp13_q001_coarse_20260425_nd2

if command -v python3 >/dev/null 2>&1; then
  q_and_p_windows="$(python3 -c 'import base64, sys; print(base64.b64decode(sys.argv[1]).decode(), end="")' MC4wMTAwfDAuMjIwMCwwLjIzMDAsMC4yNDAwLDAuMjUwMCwwLjI2MDAsMC4yNzAwLDAuMjgwMCwwLjI5MDAsMC4zMDAwLDAuMzEwMCwwLjMyMDA=)"
else
...
master_run_root=/home/DATA1/users/yuany/.single_shot/runs/3d_toric_exp13_q001_coarse_20260425_nd2
...
while IFS='|' read -r syndrome_error_probability data_error_probabilities; do
  ...
  current_seed_base=$(( 2026042513 + q_index * 1000000000 ))
  ...
  conda run -n 11 python src/production_chunked_scan.py submit     --run-root "$run_root"     --code-family 3d_toric     --workers "$workers"     --chunk-size 1     --num-disorder-samples-total 16     --data-error-probabilities "$data_error_probabilities"     --lattice-sizes 3\,4\,5     --syndrome-error-probability "$syndrome_error_probability"     --num-burn-in-sweeps 800     --num-sweeps-between-measurements 6     --num-measurements-per-disorder 1536     "${submit_extra_args[@]}"     --seed-base "$current_seed_base"     --burn-in-scaling-reference-num-qubits 18     "${burn_in_cap_args[@]}"     --output-stem "$output_stem"     --common-random-disorder-across-p     --git-commit-sha 5982451bf586e55adf4bddd729c9f4b74184abd5

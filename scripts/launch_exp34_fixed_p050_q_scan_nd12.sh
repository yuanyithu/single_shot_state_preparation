...
MASTER_RUN_ID="${MASTER_RUN_ID:-3d_toric_exp34_fixed_p050_q000_080_L34567_corrected_observable_${RUN_TIMESTAMP}}"
LOCAL_RUN_ID="${LOCAL_RUN_ID:-exp34_fixed_p050_q000_080_L34567_corrected_observable_${RUN_DATE}_nd12}"
...
Q_VALUES="${Q_VALUES:-0.0000,0.0100,0.0200,0.0300,0.0400,0.0500,0.0600,0.0700,0.0800}"
...
SEED_BASE_ND1="${SEED_BASE_ND1:-202605181}"
SEED_BASE_ND2="${SEED_BASE_ND2:-202605182}"
RESUME="${RESUME:-0}"
DRY_RUN="${DRY_RUN:-0}"
...
require_no_tracked_worktree_changes() {
  ...
  tracked_status="$(
    git -C "$PROJECT_ROOT" status --porcelain --untracked-files=no -- \
      src \
      scripts/launch_exp34_fixed_p050_q_scan_nd12.sh \
      scripts/collect_exp34_fixed_p050_q_scan_nd12.sh
  )"
...
log_msg "starting exp34 corrected-observable fixed-p q scan host=$host host_run_id=$host_run_id cpu_count=$cpu_count workers=$workers resume=$resume"
...
for lattice_size in "${lattice_sizes[@]}"; do
  ...
  for q_value in "${q_values[@]}"; do
    ...
    output_stem="scan_result_L${lattice_size}_p${fixed_p_tag}_q${q_tag}_exp34_corrected_observable_bplsd_common_random"
    submit_args=(
...
log_msg "all exp34 corrected-observable fixed-p q scan jobs completed host=$host host_run_id=$host_run_id"
EOF_RUNNER
  }
}
...
launch_one_host() {
  ...
  runner_path="$run_root/run_exp34_fixed_p050_q_scan.sh"
  screen_name="ssprep_exp34_${RUN_TIMESTAMP}_${tag}"

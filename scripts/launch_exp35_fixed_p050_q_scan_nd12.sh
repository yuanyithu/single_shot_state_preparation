...
MASTER_RUN_ID="${MASTER_RUN_ID:-3d_toric_exp35_joint_pq_adaptive_pt_fixed_p050_q080_230_L3456_${RUN_TIMESTAMP}}"
LOCAL_RUN_ID="${LOCAL_RUN_ID:-exp35_joint_pq_adaptive_pt_fixed_p050_q080_230_L3456_${RUN_DATE}_nd12}"
...
NUM_MEASUREMENTS_PER_DISORDER="${NUM_MEASUREMENTS_PER_DISORDER:-4096}"
...
ADAPTIVE_PT_ROUNDS="${ADAPTIVE_PT_ROUNDS:-3}"
...
SEED_BASE_ND1="${SEED_BASE_ND1:-202605241}"
SEED_BASE_ND2="${SEED_BASE_ND2:-202605242}"
RESUME="${RESUME:-0}"
DRY_RUN="${DRY_RUN:-0}"
...
require_no_tracked_worktree_changes() {
  ...
  tracked_status="$(
    git -C "$PROJECT_ROOT" status --porcelain --untracked-files=no -- \
      src \
      scripts/launch_exp35_fixed_p050_q_scan_nd12.sh \
...
log_msg "starting exp35 fixed-p q scan host=$host host_run_id=$host_run_id cpu_count=$cpu_count workers=$workers resume=$resume"
log_msg "grid L=$lattice_sizes_csv q=$q_values_csv fixed_p=$fixed_p adaptive_rounds=$adaptive_pt_rounds"
...
for lattice_size in "${lattice_sizes[@]}"; do
  ...
  for q_value in "${q_values[@]}"; do
    ...
    output_stem="scan_result_L${lattice_size}_p${fixed_p_tag}_q${q_tag}_exp35_joint_pq_adaptive_pt_r${adaptive_pt_rounds}"
    submit_args=(
...
log_msg "all exp35 fixed-p q scan jobs completed host=$host host_run_id=$host_run_id"
EOF_RUNNER
  }
}
...
launch_one_host() {
  ...
  runner_path="$run_root/run_exp35_fixed_p050_q_scan.sh"
  screen_name="ssprep_exp35_${RUN_TIMESTAMP}_${tag}"
...
main() {
  ...
  printf 'MASTER_RUN_ID=%s\nLOCAL_RUN_ID=%s\nLOCAL_OUTPUT_DIR=%s\nCOMMIT_SHA=%s\nADAPTIVE_PT_ROUNDS=%s\nRESUME=%s\n' \
    "$MASTER_RUN_ID" "$LOCAL_RUN_ID" "$local_output_dir" "$COMMIT_SHA" "$ADAPTIVE_PT_ROUNDS" "$RESUME"

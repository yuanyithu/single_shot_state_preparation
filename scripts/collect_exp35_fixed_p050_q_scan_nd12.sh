...
RUN_ADAPTIVE_SUMMARY="${RUN_ADAPTIVE_SUMMARY:-1}"
...
main() {
  ...
  for index in "${!hosts[@]}"; do
    ...
    tag="$(host_tag "$host")"
    if [[ -z "$host_tags_csv" ]]; then
      host_tags_csv="$tag"
    else
      host_tags_csv="${host_tags_csv},${tag}"
    fi
    ...
    scp "$REMOTE_LOGIN:${REMOTE_BASE}/logs/${host_run_id}.log" "$OUTPUT_DIR/logs/${host_run_id}.log" || true
  ...
  if [[ "$RUN_ADAPTIVE_SUMMARY" == "1" ]]; then
    summary_args=(
      conda run -n 12 python "$PROJECT_ROOT/scripts/summarize_exp35_adaptive_pt_chinese.py"
      --input-dir "$OUTPUT_DIR"
      --output-dir "$OUTPUT_DIR/adaptive_pt_summary"
    )
    if [[ "$ALLOW_PARTIAL" == "1" ]]; then

...
MASTER_RUN_ID="${MASTER_RUN_ID:-3d_toric_exp32_fixed_p050_q000_075_L34567_${RUN_TIMESTAMP}}"
LOCAL_RUN_ID="${LOCAL_RUN_ID:-exp32_fixed_p050_q000_075_L34567_${RUN_DATE}_nd23}"
REMOTE_BASE="${REMOTE_BASE:-/home/DATA1/users/yuany/.single_shot}"
REMOTE_HOSTS="${REMOTE_HOSTS:-nd-2,nd-3}"
...
SEED_BASE_ND2="${SEED_BASE_ND2:-202605142}"
SEED_BASE_ND3="${SEED_BASE_ND3:-202605143}"
RESUME="${RESUME:-0}"
DRY_RUN="${DRY_RUN:-0}"
...
require_no_tracked_worktree_changes() {
  ...
  tracked_status="$(git -C "$PROJECT_ROOT" status --porcelain --untracked-files=no)"
  if [[ -n "$tracked_status" ]]; then
    echo "Tracked working tree changes are not included in git archive deployment:" >&2
...
verify_remote_env() {
  local host="$1"
  ssh yuany "ssh ${host} 'set -euo pipefail; hostname; nproc; command -v screen >/dev/null; command -v conda >/dev/null; export CONDA_NO_PLUGINS=true; conda run -n 11 python -c \"import importlib.util, numpy, matplotlib; print(\\\"python_env_ok=1\\\"); print(\\\"numba_available=\\\" + str(importlib.util.find_spec(\\\"numba\\\") is not None))\"'"
}
...
conda run -n 11 python -c "import importlib.util, numpy, matplotlib; print('python_env_ok=1'); print('numba_available=' + str(importlib.util.find_spec('numba') is not None))"
...
log_msg "starting exp32 fixed-p q scan host=$host host_run_id=$host_run_id cpu_count=$cpu_count workers=$workers resume=$resume"
...
for lattice_size in "${lattice_sizes[@]}"; do
  ...
  for q_value in "${q_values[@]}"; do
    ...
    output_stem="scan_result_L${lattice_size}_p${fixed_p_tag}_q${q_tag}_exp32_fixed_p_q_scan_common_random"
    submit_args=(
...
log_msg "all exp32 fixed-p q scan jobs completed host=$host host_run_id=$host_run_id"
EOF_RUNNER
  }
}
...
launch_one_host() {
  ...
  runner_path="$run_root/run_exp32_fixed_p050_q_scan.sh"

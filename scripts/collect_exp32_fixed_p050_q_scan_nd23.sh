...
LOCAL_RUN_ID="${LOCAL_RUN_ID:-exp32_fixed_p050_q000_075_L34567_20260514_nd23}"
...
main() {
  ...
  if [[ -z "$HOST_RUN_IDS" ]]; then
    echo "Set HOST_RUN_IDS to comma-separated nd-2,nd-3 run ids." >&2
    exit 2
  fi
  ...
  if [[ "$RUN_ANALYSIS" == "1" ]]; then
    analysis_args=(
      conda run -n 12 python "$PROJECT_ROOT/src/analyze_exp32_fixed_p_q_scan.py"
